...
def delete_attachment(
    ...
    return None
@router.get(
    "/responsible/reports/{token}/attachments/{attachment_id}/file",
...
def responsible_view_attachment(
    ...
    report = (
        ...
        .filter(ExpenseReport.public_token == token)
        .first()
    )
    if not report:
        raise HTTPException(status_code=404, detail="Invalid token")

    # 2️⃣ Find attachment linked to this report
    attachment = (
        db.query(Attachment)
        .join(ExpenseItem)
        .filter(
